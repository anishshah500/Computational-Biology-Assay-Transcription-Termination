...
k <- data.frame(tps = rep(c(5,10,20,60), each=nrow(data.5m.1.minima)*3),
...
pdf("boxplot_without_log_y-axis_gtf.pdf")
print(ggplot(k, aes(x=factor(tps),y=ratios,fill=factor(reps))) + geom_boxplot(notch=T) + ylim(0,1) + theme_bw() + ggtitle("Boxplot for gtf"))
dev.off()
