...
result_gtf <- t
colnames(result_gtf) <- c("gene_id","slope","r_sq","p_value")
ggplot(result_gtf,aes(slope)) + geom_histogram()

#printing the graphs
pdf("lm_statistics_gtf.pdf")
{
  print(ggplot(result_gtf,aes(as.numeric(slope))) + geom_histogram(aes(fill = ..count..),binwidth = 1) + theme_bw() + labs(x = "Slope"))
  print(ggplot(result_gtf,aes(as.numeric(r_sq))) + geom_histogram(aes(fill = ..count..),binwidth = 0.01) + theme_bw() + labs(x = "R Square"))
  print(ggplot(result_gtf,aes(as.numeric(p_value))) + geom_histogram(aes(fill = ..count..),binwidth = 0.01) + theme_bw() + labs(x = "P Value"))
}
